...
type_attribute = G.NonTerminal("<type_attribute>")
...
statement %= function + identifier + parameters + function_style, lambda h, s: FunctionNode(s[2].name, s[3], s[4])
statement %= type_ + identifier + type_def, lambda h, s: TypeNode(s[2].name, s[3][3], s[3][0], s[3][1], s[3][2])
...
# #
variable %= identifier, lambda h, s: ParameterNode(s[1].name)
variable %= identifier + colon + identifier, lambda h, s: ParameterNode(s[1].name, s[3].name)
# #
type_def %= class_block, lambda h, s: ([], 'Object', [], s[1])
type_def %= inherits + identifier + class_block, lambda h, s: ([], s[2].name, [], s[3])
type_def %= lparen + parameter_list + rparen + class_block, lambda h, s: (s[2], 'Object', [], s[4])
type_def %= lparen + parameter_list + rparen + inherits + identifier + class_block, lambda h, s: (s[2], s[5].name, [], s[6])
type_def %= lparen + parameter_list + rparen + inherits + identifier + lparen + argument_list + rparen + class_block, lambda h, s: (s[2], s[5].name, s[7], s[9])
type_def %= inherits + identifier + lparen + argument_list + rparen + class_block, lambda h, s: ([], s[2].name, s[4], s[6])
# #
...
class_declaration %= identifier + parameters + function_style, lambda h, s: FunctionNode(s[1].name, s[2], s[3])
# #
protocol_declare %= protocol + identifier + lbrack + protocol_body + rbrack, lambda h, s: ProtocolNode(s[2].name, s[4])
protocol_declare %= protocol + identifier + extends + identifier + lbrack + protocol_body + rbrack, lambda h, s: ProtocolNode(s[2].name, s[6], s[4])
#
protocol_body %= identifier + typed_parameters + colon + identifier + semicolon, lambda h, s: ProtocolMethodNode(s[1], s[2], s[4])
protocol_body %= identifier + typed_parameters + colon + identifier + semicolon + protocol_body, lambda h, s: [ProtocolMethodNode(s[1].name, s[2], s[3])] + s[6]
# #
typed_parameters %= lparen + rparen, lambda h, s: []
typed_parameters %= lparen + typed_parameter_list + rparen, lambda h, s: s[2]
#
typed_parameter_list %= identifier + colon + identifier, lambda h, s: [ParameterNode(s[1].name, s[2].name)] # check
typed_parameter_list %= identifier + colon + identifier + typed_parameter_list, lambda h, s: [ParameterNode(s[1].name, s[2].name)] + s[4]
# #
...
simple_expression %= identifier + destruct + simple_expression, lambda h, s: DestructiveExpression(s[1].name, s[3])
simple_expression %= identifier + period + identifier + destruct + simple_expression, lambda h, s: SelfDestructiveExpression(SelfVariableNode(True, s[3].name), s[5])
...
simple_expression %= for_ + lparen + identifier + in_ + simple_expression + rparen + expression, lambda h, s: forNode(s[3].name, s[5], s[7])
simple_expression %= new + identifier + arguments, lambda h, s: NewNode(s[2].name, s[3])
simple_expression %= disjunction, lambda h, s: s[1]
# #
declaration %= variable + equal + simple_expression, lambda h, s: ([s[1]], [s[3]]) # duda
declaration %= variable + equal + simple_expression + comma + declaration, lambda h, s: ([s[1]] + s[5][0], [s[3]] + s[5][1]) # duda
#
...
disjunction %= conjunction + or_ + disjunction, lambda h, s: OrAndExpression(s[2].name, s[1], s[3])
#
conjunction %= literal, lambda h, s: s[1]
conjunction %= literal + and_ + conjunction, lambda h, s: OrAndExpression(s[2].name, s[1], s[3])
#
literal %= proposition, lambda h, s: s[1]
literal %= not_ + literal, lambda h, s: NotExpression(s[2])
# #
proposition %= boolean, lambda h, s: s[1]
proposition %= proposition + is_ + identifier, lambda h, s: IsExpression(s[1], s[3].name)
#
boolean %= concatenation, lambda h, s: s[1]
boolean %= boolean + dequal + concatenation, lambda h, s: ComparationExpression(s[2].name, s[1], s[3])
boolean %= boolean + notequal + concatenation, lambda h, s: ComparationExpression(s[2].name, s[1], s[3])
boolean %= boolean + lequal + concatenation, lambda h, s: ComparationExpression(s[2].name, s[1], s[3])
boolean %= boolean + gequal + concatenation, lambda h, s: ComparationExpression(s[2].name, s[1], s[3])
boolean %= boolean + lesst + concatenation, lambda h, s: ComparationExpression(s[2].name, s[1], s[3])
boolean %= boolean + greatt + concatenation, lambda h, s: ComparationExpression(s[2].name, s[1], s[3])
#
...
arithmetic_expression %= arithmetic_expression + plus + module, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
arithmetic_expression %= arithmetic_expression + minus + module, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
#
module %= product, lambda h, s: s[1]
module %= module + modulus + product, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
# #
product %= monomial, lambda h, s: s[1]
product %= product + times + monomial, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
product %= product + divide + monomial, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
product %= product + int_divide + monomial, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
# #
monomial %= pow_, lambda h, s: s[1]
monomial %= minus + monomial, lambda h, s: AritmethicExpression(s[1].name, NumberNode(0), s[2])
#
pow_ %= high_hierarchy_object, lambda h, s: s[1]
pow_ %= pow_ + power_asterisk + high_hierarchy_object, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
pow_ %= pow_ + power + high_hierarchy_object, lambda h, s: AritmethicExpression(s[2].name, s[1], s[3])
# #
high_hierarchy_object %= object_exp, lambda h, s: s[1]
high_hierarchy_object %= high_hierarchy_object + as_ + type_attribute, lambda h, s: asNode(s[1], s[3].name)
#
type_attribute %= object_exp, lambda h, s: s[1]
type_attribute %= type_attribute + period + identifier, lambda h, s: SelfVariableNode(True, s[3].name)
type_attribute %= type_attribute + period + identifier + arguments, lambda h, s: TypeFunctionCallNode(s[1], s[3].name, s[4])
...
object_exp %= object_exp + arguments, lambda h, s: FunctionCallNode(s[1].name, s[2])
object_exp %= lbrack + list_ + rbrack, lambda h, s: ListNode(s[2])
object_exp %= object_exp + lbrack + simple_expression + rbrack, lambda h, s: InexingNode(s[1], s[3])
object_exp %= print_ + lparen + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, s[3])
object_exp %= sin + lparen + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, s[3])
object_exp %= cos + lparen + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, s[3])
object_exp %= tan + lparen + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, s[3])
object_exp %= sqrt + lparen + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, s[3])
object_exp %= exp + lparen + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, s[3])
object_exp %= log + lparen + simple_expression + comma + simple_expression + rparen, lambda h, s: FunctionCallNode(s[1].name, [s[3]] + [s[5]]) # duda
object_exp %= base + lparen + rparen, lambda h, s: FunctionCallNode(s[1].name, [])
object_exp %= range_ + lparen + rparen, lambda h, s: FunctionCallNode(s[1].name, [])

list_ %= simple_expression, lambda h, s: [s[1]]
list_ %= simple_expression + comma + list_, lambda h, s: [s[1]] + s[3]
list_ %= simple_expression + list_comprehension + identifier + in_ + simple_expression, lambda h, s: ImplicitListNode(s[1], s[3].name, s[5])
